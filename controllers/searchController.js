  ...
  try {
    ...
    try {
      ...
      for (const mainResult of mainResults) {
        ...
        try {
          ...
          console.error(
            `Unable to fetch app details for appID: ${mainResult.appId}`,
          );
        }
    ...
    if (permissions) {
      ...
      const permissionsResults =
        await permissionsController.fetchPermissions(uniqueResults);
    ...
    if (
      emailMappings[pushQuery] &&
      Array.isArray(emailMappings[pushQuery]) &&
      emailMappings[pushQuery].length > 0
    ) {
              ...
              transporter.sendMail(
                {
                  ...
                  to: userEmail,
                  subject:
                    "[SMAR Tool] Your App Store/Play Store Data is Ready!",
...
const addEmailNotification = (req, res) => {
  cors()(req, res, () => {
    try {
      ...
      if (!emailMappings[queryId])
        emailMappings[queryId] = []; // initialize email array

      // Avoid duplicates
      if (!emailMappings[queryId].includes(email))
        emailMappings[queryId].push(email);
